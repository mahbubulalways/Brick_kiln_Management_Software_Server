...
exports.formatCustomerDataWithPrevDue = exports.formatCustomerData = void 0;
const formatCustomerData = (customers) => {
    return customers.map((customer) => {
        // মোট কেনা quantity
        const totalPurchasedQuantity = customer.challans.reduce((challanTotal, challan) => {
            return (challanTotal +
                challan.items.reduce((itemTotal, item) => itemTotal + item.quantity, 0));
        }, 0);
        // মোট delivered quantity
        const totalDeliveredQuantity = customer.challans.reduce((challanTotal, challan) => {
            return (challanTotal +
                challan.deliveries.reduce((deliveryTotal, delivery) => deliveryTotal + delivery.deliveryReceived, 0));
        }, 0);
        // Due collections
        ...
        // মোট বিল
        const totalAmount = customer.customerDues.reduce((total, due) => total + Number(due.totalAmount), 0);
        // মোট payment
        const totalPaid = customer.customerDues.reduce((total, due) => total + Number(due.paidAmount), 0) + totalDueCollection;
        // টাকা বাকি
...
const formatCustomerDataWithPrevDue = (customers, previousDues, previousCollections) => {
    return customers.map((customer) => {
        // ==========================================
        ...
        // ==========================================
        const totalDueCollection = customer.dueCollections.reduce((total, due) => total + Number(due.collect), 0);
        // ==========================================
...
exports.formatCustomerDataWithPrevDue = formatCustomerDataWithPrevDue;
