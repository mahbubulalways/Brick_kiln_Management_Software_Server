...
const createNewUnloadService = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!round?.id) {
      throw new AppError(StatusCodes.NOT_FOUND, "রাউন্ড পাওয়া যায়নি।");
    }
    ...
    if (!classInfo) {
      throw new AppError(StatusCodes.NOT_FOUND, "কোনো শ্রেণি পাওয়া যায়নি।");
    }
    ...
    if (!brickStock) {
      throw new AppError(StatusCodes.NOT_FOUND, "ইটের স্টক পাওয়া যায়নি।");
    }
...
const deleteUnloadService = async (user: TAuthUser, id: string) => {
  const isExist = await prisma.unload.findFirst({
    where: { id, round: { vataId: user.vataId } },
  });
  if (!isExist) {
    throw new AppError(StatusCodes.NOT_FOUND, "আনলোডের তথ্য পাওয়া যায়নি");
  }
  const result = await prisma.unload.update({
    where: { id, round: { vataId: user.vataId } },
    data: { isDeleted: true },
  });
  return result;
