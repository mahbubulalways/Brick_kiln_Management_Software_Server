...
const checkSubdomainExistController = catchAsync(async (req, res) => {
  ...
  if (result) {
    ...
    throw new AppError(StatusCodes.BAD_REQUEST, "ডোমেইনটি যাচাই করা যায়নি");
  }
});
...
const getMyVataInformationController = catchAsync(async (req, res) => {
  ...
  if (result) {
    ...
    throw new AppError(StatusCodes.NOT_FOUND, "ভাটার তথ্য পাওয়া যায়নি");
  }
});
...
const getVataExpirityController = catchAsync(async (req, res) => {
  const user = req.user as TAuthUser;
  const result = await VataService.getVataExpirityService(user);
  sendResponse(res, {
    statusCode: 200,
    success: true,
